...
APPLY
  ...
  demo_start_process CHAT_PID forge-chat "$WORK/forge-chat.log" 18056 -- \
    "$WORK/forge-chat" --mcp http://127.0.0.1:18055 \
      ...
      --approval-url "${PWA_URL:-http://127.0.0.1:18054/}" --listen 127.0.0.1:18056
  PIDS+=("$CHAT_PID")
  demo_wait_http_ready forge-chat "$CHAT_PID" "http://127.0.0.1:18056/" "$WORK/forge-chat.log"

  echo
  # Two URLs, and the order they are printed in is the order they are useful
  # in. Printed side by side with equal weight, the approval link invites
  # someone to open it first, find an empty page, and reasonably conclude the
  # demo is broken -- nothing is held until the model asks for something.
  ok "START HERE  ->  http://127.0.0.1:18056/"
  echo "              the chat. Tell it something is wrong with your connector."
  echo
  if [ -n "$PWA_URL" ]; then
    ok "then, once it asks for an operation, decide it here:"
    echo "   $PWA_URL"
    echo
    echo "   That is the real ForgeOps Approval PWA. It is EMPTY until the model"
    echo "   requests something -- that is the demo working, not a fault. The link"
    echo "   carries a one-time session for the customer-approver identity and is"
    echo "   stripped from the address bar as soon as the page adopts it."
  else
    ok "then, once it asks for an operation, decide it at http://127.0.0.1:18054/"
    echo "   (empty until the model requests something)"
  fi
  cat <<CHAT

Something is wrong with the connector. Open the chat above and ask the model
what, then watch three things:

  - a read runs with no approval and no friction;
  - the operation it decides on is HELD, and only then does the approval page
    have anything in it;
